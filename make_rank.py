...
categories = [
    ...
    "env-1-midlipperiness",
    "env-2-midlipperiness",
    "env-3-midlipperiness",
    "env-4-midlipperiness",
...
fig_error.write_image(
    "ranking_error.png", format="png", scale=1, width=1200, height=800, engine="kaleido"
)
...
fig_ranking.write_image(
    "ranking_visualization.png",
    format="png",
    scale=1,
    width=1200,
    height=800,
...
for b_idx in np.argsort(ranking_errors):
    pred_rank_tuple = tuple(
        ranking_results[:, b_idx]
    )  # Convert ranking array to a tuple for uniqueness
...
for i, b_idx in enumerate(best_b_indices):
    ...
    fig_comparison.update_layout(
        ...
        template="plotly_white",
        yaxis=dict(
            autorange="reversed", range=[-1, len(categories)]
    ...
    fig_comparison.write_image(
        filename, format="png", scale=1, width=1200, height=800, engine="kaleido"
    )
