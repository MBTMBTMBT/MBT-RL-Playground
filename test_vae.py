...
latent_dim = 1
vae = VAE(in_channels=3, latent_dim=1, input_size=(60, 80), hidden_dims=[256, 512, 1024],)
...
latent_dim = st.sidebar.number_input("Latent Dimension", min_value=1, max_value=512, value=latent_dim, step=1)
...
st.markdown("Interactively adjust latent variables to explore the VAE's generative capabilities.")
...
generated_image = vae.decode(latent_input_tensor).detach().numpy()[0]  # Assume decode outputs (C, H, W)
...
st.image(generated_image, caption="Generated Image from Latent Variables", use_column_width=True)
