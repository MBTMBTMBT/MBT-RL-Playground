...
def run_experiment(args):
    ...
    if env_id in CUSTOM_ENVS:
        envs = [
            CUSTOM_ENVS[env_id](**(group["train_env_params"][i]))
            for i in range(len(group["train_env_params"]))
        ]
    ...
    # Training
    with tqdm(
        total=total_steps, desc=f"[{group_name}] Run {run_id}", leave=False
    ) as pbar:
        ...
        while current_steps < total_steps:
            ...
            while not done:
                if current_steps % curriculum_steps == 0:
                    ...
                    epsilon = epsilon_start
                if (
                    current_steps % curriculum_steps == 0
                    and current_steps > 0
                    and len(envs) > 1
                ):
                ...
                if current_steps % test_per_num_steps == 0:
                    ...
                    for _ in range(test_runs):
                        ...
                        while not test_done:
                            test_action = [
                                np.argmax(
                                    agent.get_action_probabilities(
                    ...
                    try:
                        ...
                        if np.isnan(average_kl):
                            ...
                            if reset_old_agent_times > 1:
                                print(
                                    "Warning, prior agent is NaN. Resetting prior agent."
                                )
                    except ValueError:
                        average_kl = 0.0

                    pbar.set_description(
                        f"[{group_name}] Run {run_id} | "
    ...
    q_table_path = os.path.join(
        save_dir, f"{group_name}_run_{run_id}_q_table_final.csv"
    )
    agent.save_q_table(q_table_path)
    if agent_type == "dqn":
        q_model_path = os.path.join(
    ...
    count_path = os.path.join(
        save_dir,
        f"{group_name}_run_{run_id}_count_final.csv",
    )
    env.export_counts(count_path)
    ...
    if current_steps % test_per_num_steps >= test_per_num_steps // 2:
        test_rewards.append(
            (current_steps, np.mean(final_test_rewards))
        )  # Append the final test result to test_rewards
        try:
            policy = compute_action_probabilities(
            ...
            old_policy = compute_action_probabilities(
                merge_q_table_with_counts(
                    old_agent.save_q_table(), env.export_counts()
                ),
                strategy="softmax",
            )
            average_kl = compute_average_kl_divergence_between_dfs(
    ...
    training_data_path = os.path.join(
        save_dir, f"{group_name}_run_{run_id}_training_data.csv"
    )
    pd.DataFrame(test_rewards, columns=["Step", "Avg Test Reward"]).to_csv(
...
def run_all_experiments(experiment_groups, save_dir, max_workers):
    ...
    for group in experiment_groups:
        group_name = group["group_name"]
        total_steps = group["total_steps"]
        all_training_results, all_kl_divergences, all_testing_results = zip(
...
if __name__ == "__main__":
    ...
    fig = sp.make_subplots(
        rows=1, cols=1, subplot_titles=["Training Results Across Experiment Groups"]
    )

    for i, (
        group_name,
        (avg_rewards, std_rewards, avg_kls, std_kls, steps, avg_test_reward),
    ) in enumerate(aggregated_results.items()):
        # Plot training curve
        ...
        trace_std = go.Scatter(
            ...
            y=[v + s for v, s in zip(avg_rewards, std_rewards)]
            + [v - s for v, s in zip(avg_rewards, std_rewards)][::-1],
            fill="toself",
        ...
        trace_test = go.Scatter(
            ...
            line=dict(
                dash=["dash", "dot", "dashdot", "longdash"][i % 4], color="black"
            ),
        )
