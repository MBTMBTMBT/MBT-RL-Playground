...
class Qtableagent:
    ...
    def print_q_table_info(self) -> None:
        ...
        for key, value in list(self.q_table.items())[
            :5
        ]:  # Print first 5 entries for preview
    ...
    @classmethod
    def load_q_table(
        cls, file_path: str = None, df: pd.DataFrame = None
    ) -> "Qtableagent":
        """
        ...
        state_columns = sorted(
            [col for col in df.columns if col.startswith("state_dim_")],
            key=lambda x: int(x.split("_")[-1]),
        )
        action_columns = sorted(
            [col for col in df.columns if col.startswith("action_dim_")],
            key=lambda x: int(x.split("_")[-1]),
        )
    ...
    def get_action_probabilities(
        ...
        q_values = np.array(
            [
                self.q_table.get((state_key, tuple(action)), 0.0)
    ...
    def _generate_all_possible_actions(self) -> List[List[int]]:
        ...
        return [
            list(action)
            for action in product(*[range(dim) for dim in self.action_space])
        ]
    ...
    def update(
        ...
        if done:
            ...
            best_next_action_value = max(
                [
                    self.q_table.get((next_state_key, tuple(a)), 0.0)
...
if __name__ == "__main__":
    ...
    agent = Qtableagent(
        action_space=[len(action_ranges[i]) for i in range(len(action_ranges))]
    )
    ...
    for episode in range(1000):
        ...
        while not done:
            action = random.choice(
                agent._generate_all_possible_actions()
            )  # Choose a random action
            next_state, reward, done, truncated, _ = wrapped_env.step(
                action[0]
            )  # Use the first action dimension for CartPole
    ...
    for r in range(
        1, len(state_features) + 1
    ):  # r is the number of features in each combination
        for feature_combination in combinations(state_features, r):
            ...
            print(
                f"Mutual Information for Features {feature_combination} with action_dim_0: {mi}"
            )
    ...
    kl = compute_average_kl_divergence_between_dfs(
        merged_df_, merged_df, weighted_by_visitation=True
    )
    ...
    print(
        "Action probabilities (greedy):",
        loaded_agent.get_action_probabilities(test_state, strategy="greedy"),
    )
    print(
        "Action probabilities (softmax):",
        loaded_agent.get_action_probabilities(
            test_state, strategy="softmax", temperature=1.0
        ),
    )
