...
def run_2_stage_cl_training(
    ...
    test_envs = (
        [prior_test_env, target_test_env]
        if not prior_env_idx == -1
        else [target_test_env]
    )
    save_paths = (
        [prior_save_path, target_save_path]
        if not prior_env_idx == -1
        else [target_save_path]
    )
    save_path = (
        configs["save_path"] + f"-{prior_env_desc}-{target_env_desc}-id-{run_id}"
    )
    ...
    for idx, env in enumerate(envs):
        ...
        while sample_step_count < configs["exploit_policy_training_steps"]:
            ...
            for t in range(configs["exploit_policy_test_episodes"]):
                ...
                while not test_done:
                    ...
                    (
                        ...
                        test_truncated,
                        _,
                    ) = test_envs[idx].step(test_action)
                    if t == 0:
            ...
            if not first_test:
                if (
                    avg_test_reward < best_avg_reward
                    or best_avg_reward >= configs["success_threshold"]
                ):
...
def run_all_2_stage_cl_training_and_plot(
    ...
    for (
        ...
        grouped_results[task_name][prior_env_idx][target_env_idx][
            "target_test_results"
        ].append(target_test_results)
        grouped_results[task_name][prior_env_idx][target_env_idx]["test_steps"].append(
            test_steps
        )
        grouped_results[task_name][prior_env_idx][target_env_idx][
            "prior_end_steps"
    ...
    for task_name, prior_envs in aggregated_results.items():
        ...
        for prior_env_idx, target_envs in prior_envs.items():
            for target_env_idx, subtask_data in target_envs.items():
                ...
                mean_target_test_results = np.array(
                    subtask_data["mean_target_test_results"]
                )
                std_target_test_results = np.array(
                    subtask_data["std_target_test_results"]
                )
                ...
                prior_env_desc = (
                    "scratch"
                    if prior_env_idx == -1
                    else get_envs_discretizers_and_configs(task_name, prior_env_idx)[2]
                )
                target_env_desc = get_envs_discretizers_and_configs(
                ...
                fig_target_test.add_trace(
                    go.Scatter(
                        ...
                        + (mean_target_test_results - std_target_test_results)[
                            ::-1
                        ].tolist(),
                        fill="toself",
                        fillcolor=f"rgba({int(colors[color_idx][1:3], 16)}, {int(colors[color_idx][3:5], 16)}, {int(colors[color_idx][5:], 16)}, 0.2)",
        ...
        fig_integral.add_trace(
            go.Bar(
                ...
                text=[
                    f"{val:.3f}" for val in bar_means_target_test
        ...
        print(
            f"Saved target test evaluation plot for {task_name} at {plot_path_target_test}"
        )

        plot_path_integral = (
            save_path + f"_training_integral_target_cl-{target_env_desc}.png"
        )
        fig_integral.write_image(plot_path_integral, scale=2)
        print(
            f"Saved integral target test performance plot for {task_name} at {plot_path_integral}"
        )

        plot_path_success_steps = (
            save_path + f"_training_success_steps_cl-{target_env_desc}.png"
        )
        fig_success_steps.write_image(plot_path_success_steps, scale=2)
        print(
            f"Saved success steps performance plot for {task_name} at {plot_path_success_steps}"
        )
