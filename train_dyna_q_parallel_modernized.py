...
from dyna_q_task_configs_modernized import (
    get_envs_discretizers_and_configs as tabular_configs,
)
from dyna_q_task_configs_modernized_deep import (
    get_envs_discretizers_and_configs as deep_configs,
)
from dyna_q_task_configs_modernized_real_env import (
    get_envs_discretizers_and_configs as real_env_configs,
)
...
def run_experiment(task_name: str, run_id: int, init_group: str):
    ...
    init_distribution = (
        configs["init_groups"][init_group] if init_group != "baseline" else None
    )
    ...
    if configs["use_deep_agent"]:
        agent = DeepDynaQAgent(
            ...
            min_cut_max_flow_search_space=configs["landmark_params"][
            ...
            init_state_reward_prob_below_threshold=configs["landmark_params"][
                "init_state_reward_prob_below_threshold"
            ],
            quality_value_threshold=configs["landmark_params"][
                "quality_value_threshold"
            ],
            take_done_states_as_targets=configs["landmark_params"][
    ...
    for sample_step in sample_steps:
        if (
            not init_group == "baseline"
            and "explore_policy_exploit_policy_ratio" in configs[sample_step]
        ):
        ...
        train_from_real_env = (
            configs[sample_step]["train_from_real_environment"]
            or init_group == "baseline"
        )
        ...
        while current_step < num_steps_to_sample:
            ...
            if (
                sample_step_count % configs["explore_policy_training_per_num_steps"]
                == 0
            ...
            if (sample_step_count + 1) % configs["save_per_num_steps"] == 0 or (
                ...
                if configs["save_mdp_graph"]:
                    try:
                        agent.transition_table_env_t.save_mdp_graph(
                            graph_path, use_encoded_states=False
                        )
